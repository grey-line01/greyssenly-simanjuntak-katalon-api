...
def response = WS.sendRequest(findTestObject('Booking/GetBookingName'))

if (response.getStatusCode() == 200) {
    def bookingNames = []

    for (int i = 1; i <= 4; i++) {
        def bookingName = [('bookingname') : i]

        bookingNames.add(bookingName)
    }
    
    def jsonResponse = new JsonBuilder(bookingNames).toPrettyString()
